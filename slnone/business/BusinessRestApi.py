    ...
    def All_loan_refCode_ApprovedCount(self, request, pk=None):
      ...
      with ThreadPoolExecutor() as executor:
       if not request.GET.get('date'):
        ...
        # goldCount=executor.submit(lambda: Goldloanapplication.objects.filter(Q(dsaref_code=pk) | Q(franrefCode=pk) | Q(empref_code=pk),applicationverification__verification_status='Approved').count())
        # otherCount=executor.submit(lambda: otherloans.objects.filter(Q(dsaref_code=pk) | Q(franrefCode=pk) | Q(empref_code=pk),applicationverification__verification_status='Approved').count())
       else:
        print(date_obj1)
    ...
    def All_Frloan_refCode_ApprovedCount(self, request, pk=None):
      ...
      with ThreadPoolExecutor() as executor:
       if not request.GET.get('date'):
        ...
        # goldCount=executor.submit(lambda: Goldloanapplication.objects.filter(dsaref_code=None,franrefCode=pk,empref_code=None,applicationverification__verification_status='Approved').count())
        # otherCount=executor.submit(lambda: otherloans.objects.filter(dsaref_code=None,franrefCode=pk,empref_code=None,applicationverification__verification_status='Approved').count())
       else:
        print(date_obj1)
