...
# BasicdetailGettingRecordLogic
            ...
            # print(request.session.get('BusinessLoanApplicationId'),"request.session.get('BusinessLoanApplicationId')")
            try:
...
def salesLogic(request,refCode,businessObj):
                print(f"{settings.SALES_URL}sa/api/getDsa/{refCode}")
...
def demoPurpose(request):
    ...
    print(date.today(),record.pan_card_number)
