    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if otherbasicdetail:
                ...
                print("No matching basic detail found for this mobile number.")
...
def superAdmin(request,refCode,loan):
                print("Superadmin....")
                print(f"{settings.SUPERADMIN_URL}/superadmin/app1/getAdmin/{refCode}")
                ...
                if getDsa.status_code == 200:
                    ...
                    print("Superadmin2....")
                    print(f"{settings.SUPERADMIN_URL}/superadmin/app1/adminApplicationViewsets")
                    response = requests.post(f"{settings.SUPERADMIN_URL}/superadmin/app1/adminApplicationViewsets", json=context)
                    # print(f"{settings.DSA_URL}")
                    if response.status_code != 200 or response.status_code != 201:
                        print("Superadmin3....",f"{response.status_code}---{response.text}")
                        return HttpResponse(f"Invalid Data..{response.status_code}---{response.text}")
                else:
                    print("Superadmin4....")
...
def disbursement_summary(request):
    ...
    for details in details_list:
        print(f"Details: {details}, Verification ID: {details.verification.application_id if details.verification else 'No Verification'}")
...
def dsa(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print('Form errors:', form.errors)  # Check what validation fails
    else:
        form = dsaform()
